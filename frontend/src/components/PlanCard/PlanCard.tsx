...
export function PlanCard({ step }: PlanCardProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 p-4 shadow-sm bg-gray-300">
      <h3 className="font-bold text-gray-900">{resource.name}</h3>
      <p className="text-sm text-black">{resource.category}</p>
      <p className="mt-2 text-sm text-black">{resource.address}</p>
